...
private LineMap generateLineMap() {
		Rectangle rect = new Rectangle(0, 0, width*TILE_SIZE, height*TILE_SIZE);
		ArrayList<Line> lines = new ArrayList<Line>();
                                        lines.add(new Line(width*TILE_SIZE, 0, 0, 0));
					lines.add(new Line(width*TILE_SIZE,0,height*TILE_SIZE,height*TILE_SIZE));
					lines.add(new Line(0,0,height*TILE_SIZE,0));
					lines.add(new Line(width*TILE_SIZE,width*TILE_SIZE,0,height*TILE_SIZE));
		for(int x=0; x < width; x++){
			for(int y=0; y < height; y++){
                            
				if(bitset[x][y]==true){
                                    System.out.println(x+"  "+y);
					lines.add(new Line((x)*TILE_SIZE, (y+1)*TILE_SIZE, (x+1)*TILE_SIZE, (y+1)*TILE_SIZE));
					lines.add(new Line((x+1)*TILE_SIZE, (y)*TILE_SIZE,x*TILE_SIZE , (y)*TILE_SIZE));
					lines.add(new Line((x)*TILE_SIZE, (y)*TILE_SIZE, (x)*TILE_SIZE, (y+1)*TILE_SIZE));
